...
select customer_type, branch, gross_income, rating
from walmart_sales where product_line in
(select product_line from walmart_sales group by product_line having avg(rating) >
(select avg(rating) as average from walmart_sales))
and branch in
(select branch from walmart_sales group by branch having avg(gross_income) >
(select avg(gross_income) as avg1 from walmart_sales));
...
/*Task 42 (BigBasket Dataset)

Find all products whose sale price is above the average sale price of all products,
but whose rating is below the maximum rating within their category. 

Display ProductName, Category, SalePrice, Rating, along with
the average sale price (overall) and the maximum category rating for comparison.*/

use internshala;
select * from bigbasket;

select c.product, c.category, c.sale_price, c.rating
from bigbasket c
where c.sale_price > (select avg(sale_price) from bigbasket)
and c.rating <
(select max(b.rating)
from bigbasket b
